...
namespace MonkeyBot.Modules
{
    ...
    public class FeedModule : MonkeyModuleBase
    {
        ...
        public async Task AddFeedUrlAsync([Summary("The name/title of the feed")] string name, [Summary("The url to the feed (Atom/RSS)")] string url, [Summary("Optional: The name of the channel where the Feed updates should be posted. Defaults to current channel")] string channelName = "")
        {
            if (name.IsEmpty())
            {
                _ = await ctx.RespondAsync("Please enter a name for the feed!").ConfigureAwait(false);
                return;
            }
            if (url.IsEmpty())
            {
                _ = await ctx.RespondAsync("Please enter a feed url!").ConfigureAwait(false);
                return;
            }
            ITextChannel channel = await GetTextChannelInGuildAsync(channelName, true).ConfigureAwait(false);
            if (channel == null)
            {
                _ = await ctx.RespondAsync("The specified channel was not found").ConfigureAwait(false);
                return;
            }
            ...
            {
                _ = await ctx.RespondAsync($"Multiple feeds were found at this url. Please be more specific:{Environment.NewLine}{string.Join(Environment.NewLine, urls)}").ConfigureAwait(false);
                return;
            }
            ...
            {
                _ = await ctx.RespondAsync("The specified feed is already in the list!").ConfigureAwait(false);
                return;
            }
            await feedService.AddFeedAsync(name, feedUrl, Context.Guild.Id, channel.Id).ConfigureAwait(false);
            _ = await ctx.RespondAsync("Feed added").ConfigureAwait(false);
        }
        ...
        public async Task RemoveFeedUrlAsync([Summary("The name or the url of the feed")] string nameOrUrl, [Summary("Optional: The name of the channel where the Feed url should be removed. Defaults to current channel")] string channelName = "")
        {
            ...
            {
                _ = await ctx.RespondAsync("The specified feed is not in the list!").ConfigureAwait(false);
                return;
            }

            await feedService.RemoveFeedAsync(nameOrUrl, Context.Guild.Id, channel.Id).ConfigureAwait(false);
            _ = await ctx.RespondAsync("Feed removed").ConfigureAwait(false);
        }
        ...
        public async Task ListFeedUrlsAsync([Summary("Optional: The name of the channel where the Feed urls should be listed for. Defaults to all channels")] string channelName = "")
        {
            ...
            {
                _ = await ctx.RespondAsync("No feeds have been added yet.").ConfigureAwait(false);
            }
            else
            {
                if (channel == null)
                {
                    ...
                    _ = await ctx.RespondAsync($"The following feeds are listed in all channels:{Environment.NewLine}{sb.ToString()}").ConfigureAwait(false);
                }
                else
                {
                    string allUrls = string.Join(Environment.NewLine, feeds.Select(x => x.feedUrl));
                    _ = await ctx.RespondAsync($"The following feeds are listed in {channel.Mention}:{Environment.NewLine}{string.Join(Environment.NewLine, feeds.Select(x => x.feedUrl))}").ConfigureAwait(false);
                }
            }
        }
